...
- **Game script** — a single `<script>` block with no modules/classes, structured as:
  ...
  - **`draw()`**: clears and redraws the canvas each frame — background dots, aliens (per-row color from `ROW_COLORS`, `drawAlien()` adds an orbit ring for circlers and extra glow for an actively-diving diver), bullets/player as glowing shapes (`glowRect` helper sets `shadowBlur` before fill), and particles.
...
Every `spawnAliens()` call sets `state.phase = 'countdown'` and `state.countdownTimer = COUNTDOWN_FRAMES` (240). While in that phase, `update()` only decrements the timer and returns — player/alien/bullet logic is frozen — until the timer hits 0 and `state.phase` flips to `'playing'`. `draw()` renders the frozen formation underneath a dimmed overlay with the current step from `COUNTDOWN_LABELS` (`'3'`, `'2'`, `'1'`, `'GO!'`, one per 60 frames) plus a `ROUND N` label, so a new round/wave is always previewed before it goes live.
